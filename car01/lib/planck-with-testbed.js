    ...
    Stage(function(stage, canvas) {
        ...
        (function() {
            ...
            drawingTexture.draw = function(ctx) {
                ...
                ctx.lineCap = "round";
                ctx.save();
                for(let x = testbed.xmin; x<=testbed.xmax; x+=10){
                    ctx.beginPath();
                    ctx.strokeStyle = 'rgb(64,64,128)';
                    ctx.moveTo(x, testbed.ymin);
                    ctx.lineTo(x, testbed.ymax);
                    ctx.stroke();
                }
                for(let y = testbed.ymin; y<=testbed.ymax; y+=10){
                    ctx.beginPath();
                    ctx.strokeStyle = 'rgb(64,64,128)';
                    ctx.moveTo(testbed.xmin, y);
                    ctx.lineTo(testbed.xmax, y);
                    ctx.stroke();
                }
                ctx.restore();
            ...
            testbed.drawText = function(p, text, color) {
                buffer.push(function(ctx, ratio) {
                    ctx.fillStyle = color;
                    ctx.save();
                ...
                drawHash += "text" + p.x + "," + p.y + "," + text + "," + color;
            };
        ...
        var lastX = 0, lastY = 0;
        var lastW = testbed.width, lastH = testbed.height;
        stage.tick(function(dt, t) {
            ...
            if (lastW !== testbed.width || lastH !== testbed.height) {
...
function Viewer(world, opts) {
    ...
    this._options.lineWidth = 8 / this._options.ratio;
    this._world = world;
